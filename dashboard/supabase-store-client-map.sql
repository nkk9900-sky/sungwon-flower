-- 지점명 → 거래처(법인명) 매핑 테이블 (계산서 발행용)
-- Supabase SQL Editor에서 실행 후, 지점명/거래처 데이터를 입력하세요.
...
COMMENT ON TABLE public.store_client_map IS '지점명 입력 시 거래처(법인명) 자동 입력용';
